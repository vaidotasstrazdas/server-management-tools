...
class TestSelfDeployCommand(unittest.TestCase):
    ...
    def test_happy_path_has_correct_notifications(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                {"text": 'Copying from "data/." to "/usr/local/share/srv/data"', "type": "info"},
                {
    ...
    def test_copies_paths(self):
        ...
        self.assertEqual(
            ...
            [
                CopyPathParams(location_from="data/.", location_to="/usr/local/share/srv/data"),
                CopyPathParams(
                    location_from="autostart.pyz", location_to="/usr/local/sbin/autostart.pyz"
                ),
                CopyPathParams(
                    location_from="configurator.pyz", location_to="/usr/local/sbin/configurator.pyz"
                ),
                CopyPathParams(
                    location_from="installer.pyz", location_to="/usr/local/sbin/installer.pyz"
                ),
            ],
        )
    ...
    def test_correct_notifications_visible_when_path_does_not_exist_case_1(self):
        self._path_existence_fail_notifications_test("data")

    def test_correct_notifications_visible_when_path_does_not_exist_case_2(self):
        self._path_existence_fail_notifications_test("autostart.pyz")

    def test_correct_notifications_visible_when_path_does_not_exist_case_3(self):
        self._path_existence_fail_notifications_test("configurator.pyz")

    def test_correct_notifications_visible_when_path_does_not_exist_case_4(self):
        self._path_existence_fail_notifications_test("installer.pyz")
    ...
    def _path_existence_fail_notifications_test(self, path: str):
        ...
        expected_notifications = [
            {"text": "Will check paths if self deployment possible", "type": "info"},
            {
                "text": f"Path {path} does not exist, probably this is not the path where self deployment should start from.",
                "type": "error",
            },
        ]
        self.assertEqual(self.notifications.params, expected_notifications)

    def _copy_fail_notifications_test(
        self, copy_sequence: str, expected_notifications: list[object]
    ):
        # Arrange
        self.file_system.copy_path_result_map[copy_sequence] = OperationResult[bool].fail("Failure")
