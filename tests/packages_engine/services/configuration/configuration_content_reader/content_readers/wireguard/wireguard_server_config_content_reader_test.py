...
from packages_engine.services.configuration.configuration_content_reader.content_readers.wireguard import WireguardServerConfigContentReader
...
class TestWireguardServerConfigContentReader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.file_system.read_text_result_map = {
            ...
            f'/usr/local/share/{self.config.server_data_dir}/data/wireguard/wg0.server.conf': OperationResult[str].succeed("""[Interface]
PrivateKey = {{SERVER_KEY}}
Address = 10.10.0.1/24
ListenPort = 51820"""),
            f'/usr/local/share/{self.config.server_data_dir}/data/wireguard/wg0.client.conf': OperationResult[str].succeed("""[Peer]
...
AllowedIPs = {{CLIENT_IP_ADDRESS}}/32"""),
            ...
            '/etc/wireguard/clients/developer.pub': OperationResult[str].succeed('developer_public_key_value'),
            '/etc/wireguard/clients/viewer.endpoint': OperationResult[str].succeed('10.10.0.3'),
            '/etc/wireguard/clients/viewer.pub': OperationResult[str].succeed('viewer_public_key_value'),
            '/etc/wireguard/clients/operator.endpoint': OperationResult[str].succeed('10.10.0.4'),
            '/etc/wireguard/clients/operator.pub': OperationResult[str].succeed('operator_public_key_value'),
        }
    ...
    def test_happy_path_configuration(self):
        ...
        # Assert
        self.assertEqual(result.data, """[Interface]
...
AllowedIPs = 10.10.0.4/32""")
    ...
    def test_fails_when_server_config_not_read(self):
        self._failed_path_test(f'/usr/local/share/{self.config.server_data_dir}/data/wireguard/wg0.server.conf')

    def test_fails_when_server_client_config_not_read(self):
        self._failed_path_test(f'/usr/local/share/{self.config.server_data_dir}/data/wireguard/wg0.client.conf')
