...
class DockerUbuntuConfigurationTask(ConfigurationTask):
    ...
    def configure(self, data: ConfigurationData) -> OperationResult[bool]:
        ...
        write_result = self.file_system.write_text('/srv/stack/docker-compose.yml', read_result.data)
    ...
    def _create_directories_if_needed(self, directories: list[str]) -> OperationResult[bool]:
        ...
        for directory in directories:
            ...
            if not create_result.success:
                self.notifications.error('\tCreating directories operation failed.')
                return create_result.as_fail()
        self.notifications.success('\tCreating directories operation succeeded.')
        
        return OperationResult[bool].succeed(True)
    
    def _create_directory_if_needed(self, directory: str) -> OperationResult[bool]:
        self.notifications.info(f'\tCreating directory "{directory}" if needed.')
        if self.file_system.path_exists(directory):
            self.notifications.success(f'\t\tDirectory "{directory}" exists. No need to create it again.')
            return OperationResult[bool].succeed(True)
        
        self.notifications.info(f'\t\tDirectory "{directory}" does not exist. Creating it now.')
        create_result = self.controller.run_raw_commands([f'sudo mkdir -p {directory}'])
        if not create_result.success:
            self.notifications.error(f'\t\tCreation of the directory "{directory}" failed.')
            return create_result.as_fail()
        self.notifications.success(f'\t\tCreation of the directory "{directory}" succeeded.')
