...
class NginxUbuntuConfigurationTask(ConfigurationTask):
    ...
    def configure(self, data: ConfigurationData) -> OperationResult[bool]:
        ...
        create_result = self.controller.run_raw_commands(["sudo mkdir -p /etc/ssl/internal-pki"])
        ...
        self.notifications.info("Reading certificate authority configuration file.")
        ca_read_result = self.file_system.read_text("/etc/ssl/internal-pki/ca.crt")
        if not ca_read_result.success or ca_read_result.data is None:
            self.notifications.error("\tReading certificate authority configuration file failed.")
            return ca_read_result.as_fail()
        self.notifications.error("\tReading certificate authority configuration file succeeded.")
